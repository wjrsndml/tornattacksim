#!/usr/bin/env tsx
import type { DamageContext } from "@/lib/fightSimulatorTypes";
	...
	for (const value of bonusData.values) {
		...
		try {
			if (isProbabilityBonus) {
				...
				// 其他特效使用常规测试
				const testConfig: Partial<DamageContext> = {};

				// 为条件特效添加特殊配置
				if (bonusType === "conditional" && "bodyPart" in bonusData) {
					testConfig.bodyPart = bonusData.bodyPart as string;
				}
				if (bonusType === "conditional" && "condition" in bonusData) {
					const condition = bonusData.condition;
					if (condition === "first_turn") {
						testConfig.turn = 1;
					} else if (condition === "low_health") {
						testConfig.attacker = createTestPlayer({ life: 200 });
					} else if (condition === "full_health") {
						testConfig.target = createTestPlayer({ life: 1000 });
					}
				}
