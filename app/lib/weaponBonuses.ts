import { WeaponBonus, WeaponCategory } from './fightSimulatorTypes'
...
export const WEAPON_BONUSES: { [key: string]: WeaponBonus } = {
  ...
  'Bleed': {
    name: 'Bleed',
    type: 'DOT',
    description: 'Chance at "Bleed" DOT effect, with damage ticks starting at 45% that of the damage dealt when proc occurs, and reducing to zero over 9 turns',
    minValue: 20,
    maxValue: 67,
  ...
  'Cripple': {
    name: 'Cripple',
    type: 'Enemy Debuff',
    description: 'Chance (%) to Cripple opponent reducing Dexterity by 25% (x3)',
    minValue: 20,
    maxValue: 58,
  ...
  'Disarm': {
    name: 'Disarm',
    type: 'Enemy Debuff',
    description: 'Disables opponents weapon for (X) turns upon hitting their hand or arm',
    minValue: 3,
    maxValue: 15,
  ...
  'Execute': {
    name: 'Execute',
    type: 'Buff',
    description: 'Instant defeat a target upon a damaging hit when opponent is below (%) life',
    minValue: 15,
    maxValue: 29,
    applicableWeapons: ['PS'],
    unit: '%'
  ...
  'Irradiate': {
    name: 'Irradiate',
    type: 'Enemy Debuff',
    description: 'Apply 1-3 hours radiation poisoning when used for the finishing hit',
    minValue: 100,
    maxValue: 100,
    applicableWeapons: ['PI'],
    unit: '%'
  ...
  'Motivation': {
    name: 'Motivation',
    type: 'Buff',
    description: 'Chance (%) to increase all stats by 10% (up to 5 of these may stack)',
    minValue: 15,
    maxValue: 35,
  ...
  'Paralyzed': {
    name: 'Paralyzed',
    type: 'Enemy Debuff',
    description: 'Adds 300 seconds "Paralyzed" effect (50% chance of missing turns)',
    minValue: 5,
    maxValue: 18,
    applicableWeapons: ['HA'],
    unit: '%'
  ...
  'Revitalize': {
    name: 'Revitalize',
    type: 'Buff',
    description: 'Chance (%) of restoring the energy spent attacking, when used for the finishing hit',
    minValue: 10,
    maxValue: 24,
    applicableWeapons: ['SM'],
    unit: '%'
  ...
  'Suppress': {
    name: 'Suppress',
    type: 'Enemy Debuff',
    description: '(%) chance to Suppress opponent causing 25% chance to miss future turns',
    minValue: 25,
    maxValue: 49,
    applicableWeapons: ['MG'],
    unit: '%'
  ...
  'Wind-up': {
    name: 'Wind-up',
    type: 'Buff',
    description: 'Increased damage (%) after spending a turn to wind up the weapon',
    minValue: 125,
    maxValue: 221,
    applicableWeapons: ['CL', 'PI', 'SL'],
    unit: '%'
...
// 根据武器类别获取可用的武器特效
export function getAvailableBonusesForWeapon(weaponCategory: string): WeaponBonus[] {
